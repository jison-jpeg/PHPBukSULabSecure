...
class AttendanceReportController extends Controller
{
    ...
    private function loadData($header, $data, $pdf){
        ...
        //     $pdf->Cell($w[0],6,$user->colleges->first()->collegeName ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[1],6,$user->department->departmentName ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[2],6,$user->last_name . ', ' . $user->first_name . ' ' . $user->middle_name, 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[3],6,$user->subjectCode ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[4],6,$user->subjectName ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[5],6,$user->sectionCode ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[6],6,$user->roomNumber ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[7],6,$user->days ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[8],6,$user->start_time ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Cell($w[9],6,$user->end_time ?? '', 'LR', 0, 'L', $fill);
        //     $pdf->Ln();
        // }
        ...
        foreach ($data as $user) {
            ...
            $pdf->Cell($w[5],6,$user->sectionCode ?? '', 'LR', 0, 'L', $fill);
            $pdf->Cell($w[6],6, 'COMLAB '. $user->roomNumber ?? '', 'LR', 0, 'L', $fill);
